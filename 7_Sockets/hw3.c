...
// server can communication with tcp socket and udp socket
...
int main(){
	...
	while(1){
		...
		while(count--){
			if(FD_ISSET(TcpSockfd,&fdvar)){ // if tcp socket
				// create thread1
				// thread1 : processing client request 
				if(pthread_create(&tid1,NULL,(void*)ProcessTcpRequest,(void*)NULL)!=0){
					perror("pthread_create");
					exit(1);
				}
			}else if(FD_ISSET(UdpSockfd,&fdvar)){ // if udp socket
				// create thread2
				// thread2 : processing client request
